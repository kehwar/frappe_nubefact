...
| SUNAT Status | `status` | Select | No | Options: Pending, Accepted, Rejected, Voided |
...
- `send_to_nubefact(name)` — planned to build `generar_comprobante` payload, submit, and persist response links/codes.
- `refresh_sunat_status(name)` — planned to call `consultar_comprobante` and update SUNAT status fields.
- `void_in_nubefact(name)` — planned to trigger `generar_anulacion` and track ticket/void status.
